...
describe('mergeVersions list of versions with github or gitlab version tags', () => {
    ...
    it('returns first correct gitlab tag', () => {
        ...
        expect(NpmUtils.mergeVersions(name, versions)).toBe('git+ssh://git@gitlab.customer.com:Target360/plugins/target360#dv_develop');
    });
    ...
    it('compares multiple github and npm versions', () => {
        ...
        expect(NpmUtils.mergeVersions(name, versions)).toBe('^7.0.2');
    });
...
describe('check isGitCommit()', () => {
    it('check `451709494a48af64a8a4876063c244edf41d2643` === true', () => {
        expect(NpmUtils.isGitCommit('451709494a48af64a8a4876063c244edf41d2643')).toBeTruthy();
    });

    it('check `8747a43780e4651542facd7b4feac7bcb8e3778d` === true', () => {
        expect(NpmUtils.isGitCommit('8747a43780e4651542facd7b4feac7bcb8e3778d')).toBeTruthy();
    });

    it('check `develop` === false', () => {
        expect(NpmUtils.isGitCommit('develop')).toBeFalsy();
    });

    it('check `v2.0.0` === false', () => {
        expect(NpmUtils.isGitCommit('v2.0.0')).toBeFalsy();
    });
    ...
    it('check `2.0.0` === false', () => {
        expect(NpmUtils.isGitCommit('2.0.0')).toBeFalsy();
    });
...
describe('check isExactVersionTag()', () => {
    it('check `develop` === false', () => {
        expect(NpmUtils.isExactVersionTag('develop')).toBeFalsy();
    });

    it('check `v2.0.0` === true', () => {
        expect(NpmUtils.isExactVersionTag('v2.0.0')).toBeTruthy();
    });

    it('check `~v2.0.0` === false', () => {
        expect(NpmUtils.isExactVersionTag('~v2.0.0')).toBeFalsy();
    });
    ...
    it('check `2.0.0` === false', () => {
        expect(NpmUtils.isExactVersionTag('2.0.0')).toBeFalsy();
    });
...
describe('check isAdvancedVersionTag()', () => {
    it('check `develop` === false', () => {
        expect(NpmUtils.isAdvancedVersionTag('develop')).toBeFalsy();
    });

    it('check `v2.0.0` === false', () => {
        expect(NpmUtils.isAdvancedVersionTag('v2.0.0')).toBeFalsy();
    });

    it('check `~v2.0.0` === true', () => {
        expect(NpmUtils.isAdvancedVersionTag('~v2.0.0')).toBeTruthy();
    });
    ...
    it('check `2.0.0` === false', () => {
        expect(NpmUtils.isAdvancedVersionTag('2.0.0')).toBeFalsy();
    });
...
describe('transform git log to version tags list', () => {
    ...
    const versionTags = NpmUtils.extractVersionsFromGitLog(gitLog);
...
describe('find highest version from list', () => {
    ...
    it('find exact version `v2.0.0`', () => {
        const targetVersion = 'v2.0.0';
        expect(NpmUtils.findHighestVersion(sourceVersions, targetVersion)).toBe('v2.0.0');
    });

    it('find patch version `~v2.0.0`', () => {
        const targetVersion = '~v2.0.0';
        expect(NpmUtils.findHighestVersion(sourceVersions, targetVersion)).toBe('v2.0.2');
    });

    it('find minor version `^v2.0.0`', () => {
        const targetVersion = '^v2.0.0';
        expect(NpmUtils.findHighestVersion(sourceVersions, targetVersion)).toBe('v2.2.2');
    });

    it('find non-existing `v4.0.0`', () => {
        const targetVersion = 'v4.0.0';
        expect(NpmUtils.findHighestVersion(sourceVersions, targetVersion)).toBeUndefined();
    });
...
describe('find max version or range version from list', () => {
    ...
    it('works if versions are all ranges', () => {
        ...
        expect(NpmUtils.findMaxVersion(versions)).toBe('^2.9.0');
    });
});
