...
class RequirementsManager {
  ...
  getRequirements() {
    const foreignRequirements = Object.keys(this.requirements).filter((req) => !this._isKnownRepo(req)).map((r) => {return [r] + ':' + this.requirements[r]})//requirements minus our repos
    const knownRequirements = Object.keys(this.requirements).filter((req) => this._isKnownRepo(req))//ourOwn requirements
